    ...
    **opt_params: parameters for optim
    ...
    for i in tqdm(range(epochs)):
        ...
        for b in range(num_batch):
            ...
            # Implements main Eqs. (2)-(4): empirical supervised loss on paired data.
...
def train_eta_xy(model,
    ...
    **opt_params: parameters for optim
    ...
    # Implements SI Eq. (13): nu_0 is represented by scalar observable quantiles
    # over the available/reference sample Yall.
    Ytail = torch.quantile(Yall, quantiles, dim=0)
    eta_loss = lambda y: wloss(y, Ytail, quantiles, p=1)

    # Implements SI Algorithms 1-2: refresh indices realizing target scalar quantiles
    # so the W1 term can be evaluated without storing all outputs.
    def update_idx(Y):
    ...
    for i in tqdm(range(epochs)):
        ...
        for b in range(num_batch):
            ...
            # Implements main Eq. (11): supervised MSE plus lambda-weighted
            # one-dimensional observable W1 penalty.
...
def train_eta_xuy(model,
    ...
    **opt_params: parameters for optim
    ...
    # Implements SI Algorithms 1-2: refresh indices realizing target scalar
    # quantiles of (g o phi)(x).
    def update_idx(Y):
        _, indices = torch.sort(Y)
        quantile_positions = (quantiles * (Y.size(0) - 1)).floor().long()
        return indices[quantile_positions]
    ...
    for i in tqdm(range(epochs)):
        ...
        for b in range(num_batch):
            ...
            # Implements main Eq. (12): samples from (g o phi)_# mu are obtained
            # by applying the scalar observable y.
            Ytail_temp = y(Utail_temp)
            eta_loss_val = eta_loss(Ytail_temp)

            # Implements main Methods Eq. (14): state MSE plus lambda-weighted
            # quantile W1 penalty on the one-dimensional observable law.
            loss = mse_loss_val + _lamb*eta_loss_val
